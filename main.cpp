...
const char kWindowTitle[] = "LD2B_06_ナガトモイチゴ_MT3_04_01";
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	Vec3 ball {1.2f, 0.0f, 0.0f};
	Vec3 anchor = {0.0f, 0.0f, 0.0f};
	float radius = 0.8f;

	float deltaTime = 1.0f / 60.0f;

	float angularVelo = 3.14f;
	float angle = 0.0f;
	...
	while (Novice::ProcessMessage() == 0) {
		...
		angle += angularVelo * deltaTime;

		ball.x = anchor.x + std::cos(angle) * radius;
		ball.y = anchor.y + std::sin(angle) * radius;
		ball.z = anchor.z;
		...
		DrawLine({anchor, ball / 2.0f} , projectionMatrix , viewportMatrix , WHITE);
		DrawSphere({ball, 0.05f} , projectionMatrix , viewportMatrix , WHITE);
